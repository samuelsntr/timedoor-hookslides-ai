...
export function createAuthRepository(db) { return { async register(username, password) { username=normalizeUsername(username); if(db.prepare('SELECT id FROM users WHERE username=?').get(username)) return null; const id=crypto.randomUUID(); db.prepare('INSERT INTO users VALUES (?,?,?,?)').run(id,username,await hash(password),new Date().toISOString()); return {id,username}; }, async login(username,password) { const row=db.prepare('SELECT * FROM users WHERE username=?').get(normalizeUsername(username)); if(!row || !(await verify(password,row.password_hash))) return null; return {id:row.id,username:row.username}; }, find(id) { return db.prepare('SELECT id,username FROM users WHERE id=?').get(id); } }; }
